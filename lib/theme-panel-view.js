...
export default class ThemePanelView {
  constructor(props) {
    ...
    atom.workspace.onDidOpen(({ uri }) => {
      ...
      this.refs.schemeSelectView.selectCurrentScheme();

      let pane = atom.workspace.paneForURI('atom://rainglow');
      // Trigger cancel when the panel is being closed
    ...
    return (
      $.div({ tabIndex: -1, className: 'padded rainglow-panel' },
        $(StyleSelectView, {
          ...
          onDidChange: (style) => this.previewTheme({ style }),
        }),
        $(SchemeSelectView, {
          ...
          onDidChange: (scheme) => this.previewTheme({ scheme }),
