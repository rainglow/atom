...
class Rainglow {
  ...
  getSyntaxVariablesContent(scheme, style) {
    let schemeName = style === 'standard' ? `${scheme}` : `${scheme}-${style}`;
  ...
  migrateSettings() {
    ...
    if (light) {
      atom.config.set(`${packageName}.style`, 'light');
      atom.config.set(`${packageName}.scheme`, light[1]);
    } else if (contrast) {
      atom.config.set(`${packageName}.style`, 'contrast');
      atom.config.set(`${packageName}.scheme`, contrast[1]);
    } else {
      atom.config.set(`${packageName}.style`, 'normal');
      atom.config.set(`${packageName}.scheme`, name);
    }
