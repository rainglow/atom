...
module.exports = class SelectThemeView {
  ...
  getThemes() {
    let schemes = atom.config.getSchema(`${this.rainglow.packageName}.scheme`).enum.map((option) => option.value);
    let styles = atom.config.getSchema(`${this.rainglow.packageName}.style`).enum.map((option) => option.value);
    let themes = [];
